...
install() {
  ...
  local hdpithemedir="${dest}/${name}${theme}-hdpi${color}"
  local xhdpithemedir="${dest}/${name}${theme}-xhdpi${color}"
  ...
  # Install index.theme
  {
    ...
    echo "Name=${name}${theme}${color}"
    echo "Comment=A dark modern design theme"
    echo "Encoding=UTF-8"
    echo ""
    echo "[X-GNOME-Metatheme]"
    echo "GtkTheme=${name}${theme}${color}"
    echo "MetacityTheme=${name}${theme}${color}"
...
uninstall() {
  ...
  local HDPI_THEME_DIR="${1}/${2}${3}-hdpi${4}"
  local XHDPI_THEME_DIR="${1}/${2}${3}-xhdpi${4}"
