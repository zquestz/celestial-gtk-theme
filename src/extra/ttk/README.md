...
Eight themes, four colors in light and dark, named like the other two-mode
extras:
...
- **The scale slider.** clam draws it from the root style, which rendered it
  invisibly as window-background on the trough. It now carries GTK's knob
  colors, white in both modes.
...
because ttk draws the trough as a single undivided element. Scrollbar geometry
is likewise clam's, a squared slider with stepper arrows, carrying GTK's
colors including the accent while the slider is dragged.
...
dark backgrounds, and exposes no option to change them. That quirk is shared
by every dark Tk theme.
...
variant into `~/.config/git/gitk` while gitk is closed. It rewrites that file
on exit, and picks the values up on next start.
