# GTK 2.0 Theme Assets

This directory contains the GTK 2.0 theme assets and tools to regenerate them.
...
- `assets-*.svg` - Source SVG files containing all assets for each color variant and mode (light/dark)
- `assets.txt` - List of asset IDs to extract from the SVG files
- `assets-*/` - Output directories containing the rendered PNG assets
- `gtkrc-*` - GTK 2.0 theme configuration files for each variant
...
- Assets are generated per color variant and theme mode (light/dark) to maintain theme consistency
